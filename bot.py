...
apihelper.proxy = {'https': "socks5://DHGuCN:BRqGKg@77.83.30.144:8000"}
book = {'Русский язык': 17, 'Литература': 16, 'Физика': 31, 'Иностранный язык': 24, 'Геометрия': 14,
        'Алгебра и начала': 28, 'Индивидуальный проект': 5, 'Физическая культура': 15, 'Химия': 4, 'Информатика': 20,
        'Астрономия': 7, 'История': 13, 'Биология': 5, 'Колебания и волны в': 2, 'Основы безопасности': 1}
...
def generate_a_link(message):
    try:
        received_link = message.text
        student_id = received_link[received_link.rindex("/") + 1:]
        try:
            int(student_id)
        except:
            ...
            return
        link = linkgen.generate_download_link(student_id)
...
def output(mes):
    ...
    for k, v in book.items():
        ...
        formatted_dict += "\n"
    bot.send_message(mes.chat.id, formatted_dict)
